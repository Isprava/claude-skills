...
**Feeds into:** `isp_tech_specs`, `isp_test_cases`, `isp_deliverable`
...
Refer to this spec for all token compliance checks. See `isp_central_ui` for the full spec. Key values:
...
- **Contrast remediation preserves brand intent.** Follow the same brand-preserving protocol as `isp_central_ui`: adjust lightness minimally, keep hue family, propose a token name.
